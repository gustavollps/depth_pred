...
class bts_encoder(nn.Module):
    ...
    def forward(self, ip):
        ...
        joint_input = (
        self.dense_features.relu(), self.dense_op_h2, self.dense_op_h4, self.dense_op_h8, self.dense_op_h16)
        return joint_input
...
class bts_decoder(nn.Module):
    ...
    def forward(self, joint_input, focal=None):
...
class BtsController:
    def __init__(self, log_directory='run_1', logs_folder='tensorboard', backprop_frequency=1):
        ...
        self.optimizer = torch.optim.Adam([{'params': self.bts.encoder.parameters(), 'weight_decay': 1e-2},
    ...
    def run_train_step(self, tensor_input, tensor_output, tensor_focal):
        ...
        if self.current_step % 100 == 0:
            self.writer.add_scalar("Loss", loss.item() * self.backprop_frequency / tensor_input.shape[0],
                                   self.current_step)
        ...
        if self.current_step % 1000 == 0:
            ...
            visual_result = (255 - torch.clamp_max(torchvision.utils.make_grid([tensor_output[0], model_output[0]]) * 5,
                                                   250)).byte()
