...
class MaxUnpooling2d_2x2(nn.Module):
    def __init__(self, channels):
        super(MaxUnpooling2d_2x2, self).__init__()
        self.unpool = nn.MaxUnpool2d(kernel_size=2)
        self.layer = pickle.load(open('tensors/' + str(channels) + 'tensor', 'rb'))

    def forward(self, x):
        temp = self.layer.unsqueeze(0)
        for i in range(len(x) - 1):
            temp = torch.cat((temp, self.layer.unsqueeze(0)), 0)
        return self.unpool(x, temp.to("cuda:0"))
...
class _ProjectUp(nn.Module):
    ...
    def _get_unpool_mask(self, x):
        ...
        if not key in self._unpool_masks:
            ...
            # unpool_mask = torch.tensor(unpool_mask, dtype=torch.bool).byte()
            # if x.is_cuda:
...
class UpSampling(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.unpool = MaxUnpooling2d_2x2(in_channels)
...
class UpConvolution(nn.Module):
    def __init__(self):
        ...
        self.md1 = _ProjectUp(1024)
        self.md2 = _ProjectUp(512)
        self.md3 = _ProjectUp(256)
        self.md4 = _ProjectUp(128)
        self.relu = nn.ReLU(inplace=True)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.conv_end = nn.Conv2d(2048, 1024, kernel_size=1, stride=1, padding=0)
        self.bn2 = norm_layer(2048)
